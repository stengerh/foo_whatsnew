...
*/
class container_window_autorelease_t : public container_window
{
public:
	class initquit_t : public initquit
	{
	public:
		void register_window(container_window_autorelease_t & ptr)
		{
			m_windows.add_item(&ptr);
		}
		void deregister_window(container_window_autorelease_t & ptr)
		{
	...
	private:
		void on_quit()
		{
			t_size i, count = m_windows.get_count();
			for (i=0; i<count; i++)
			{
				if (m_windows[i]->get_wnd())
					SendMessage(m_windows[i]->get_wnd(), WM_CLOSE, 0, 0);
			}
		}
		pfc::list_t<container_window_autorelease_t *> m_windows;
	};
